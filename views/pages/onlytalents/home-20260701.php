...
$categoryChips = TalentShortRepository::CATEGORY_LABELS;
?>
<script>document.body.classList.add('ot-tiktok-page', 'ot-landing-shorts');</script>
<!-- deploy-marker:20260701-ot-live-v1 -->
...
<nav class="ot-shorts-nav" aria-label="OnlyTalents">
    ...
    <div class="ot-shorts-nav__links">
        <a href="<?= ot_route('ot.discover') ?>"><?= __('ot.nav.discover') ?></a>
        <a href="<?= ot_route('ot.categories') ?>"><?= __('ot.nav.categories') ?></a>
    </div>
...
<section class="ot-tiktok ot-tiktok--landing" data-ot-tiktok>
    <div class="ot-tiktok__chrome ot-tiktok__chrome--minimal">
        <div class="ot-tiktok__toolbar" id="shorts-feed">
            ...
            <div class="ot-category-chips ot-category-chips--scroll" role="tablist" aria-label="<?= e(__('ot.categories.title')) ?>">
                <?php
                $allQs = '?filter=' . urlencode($filter);
                ?>
                <a href="<?= ot_route('ot.home') . $allQs ?>#shorts-feed"
                   class="ot-filter-pill <?= ($category ?? '') === '' ? 'is-active' : '' ?>">
                    <?= __('only_talents.filter.all') ?>
                </a>
                <?php foreach ($categoryChips as $slug => $chipLabel): ?>
                    <?php $chipQs = '?filter=' . urlencode($filter) . '&category=' . urlencode($slug); ?>
                    <a href="<?= ot_route('ot.home') . $chipQs ?>#shorts-feed"
                       class="ot-filter-pill <?= ($category ?? '') === $slug ? 'is-active' : '' ?>">
                        <?= e(TalentShortRepository::categoryLabel($slug)) ?>
                    </a>
                <?php endforeach; ?>
            </div>
    ...
    <?php else: ?>
        ...
        <div class="ot-tiktok__dots" data-ot-dots aria-hidden="true"></div>
