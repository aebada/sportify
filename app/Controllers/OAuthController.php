...
class OAuthController extends Controller
{
    ...
    {
        return $this->start('google', static fn (string $from, string $role) => GoogleOAuthStart::authUrl($from, $role));
    }

    public function googleCallback(): string
    {
        return $this->finish('google', function (string $code, ?string $state) {
            return GoogleOAuthStart::handleCallback($code, $state);
        });
    }
    ...
    protected function start(string $provider, callable $urlBuilder): string
    {
        ...
        try {
            // Do not send X-LiteSpeed-Purge here — on Hostinger it can abort the
            // response as an empty HTTP 500 before Location is applied.
    ...
    protected function loginOrRegister(array $profile): string
    {
        ...
        if (Database::available()) {
            ...
            if ($isNew) {
                ...
                $this->setupProfilesOnOAuthRegister((int) $user['id'], (string) ($user['role'] ?? 'player'));
            }
        }
        ...
        if ($isNew && $registering) {
            $msg = !empty($ref['referrer_name'])
                ? __('referrals.welcome_referred', ['name' => $ref['referrer_name']])
                : __('membership.register_suggest');
            flash('success', $msg);
            return $this->redirect($this->onboardingUrl());
        }
    ...
    /** Post-register landing — role home (fan → FIT-Pass, club → dashboard, etc.). */
    protected function onboardingUrl(): string
    {
        if (Auth::check() && Rbac::canAccessRoute(Auth::homeRoute())) {
            return Auth::homeUrl();
        }
    ...
    protected function setupProfilesOnOAuthRegister(int $userId, string $role): void
    {
        if ($role === 'trainer') {
