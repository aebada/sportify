<?php
declare(strict_types=1);
header('Content-Type: text/plain; charset=UTF-8');
header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
header('X-LiteSpeed-Purge: *');

$root = dirname(__DIR__);
$files = [
    $root . '/app/Core/App.php',
    $root . '/config/routes.php',
    $root . '/config/routes-20260621-memberships-v1.php',
    $root . '/config/routes-20260629-memberships-wellness-v1.php',
    $root . '/app/Controllers/OnlyTalentsDiscoveryController.php',
    $root . '/app/Support/helpers.php',
    $root . '/views/layouts/onlytalents.php',
    $root . '/views/pages/onlytalents/home-20260703.php',
    $root . '/public/assets/css/onlytalents.css',
];

foreach ($files as $file) {
    if (function_exists('opcache_invalidate') && is_file($file)) {
        @opcache_invalidate(realpath($file) ?: $file, true);
        @touch($file);
        echo 'invalidate=' . basename($file) . "\n";
    }
}
if (function_exists('opcache_reset')) {
    @opcache_reset();
    echo "reset=done\n";
}

$wellness = $root . '/config/routes-20260629-memberships-wellness-v1.php';
if (is_file($wellness)) {
    $raw = (string) file_get_contents($wellness);
    echo 'wellness_ot_home=' . (str_contains($raw, "OnlyTalentsDiscoveryController::class, 'home'") ? 'discovery' : 'legacy') . "\n";
}

$layout = $root . '/views/layouts/onlytalents.php';
echo 'layout_v5=' . (is_file($layout) && str_contains((string) file_get_contents($layout), 'ot-shorts-immersive-v5') ? 'yes' : 'no') . "\n";

$css = $root . '/public/assets/css/onlytalents.css';
echo 'css_v5=' . (is_file($css) && str_contains((string) file_get_contents($css), 'ot-shorts-immersive-v5') ? 'yes' : 'no') . "\n";
echo 'css_mtime=' . (is_file($css) ? (string) filemtime($css) : 'missing') . "\n";

require $root . '/bootstrap-20260621-authfix.php';
$router = \App\Core\App::$router;
if (\App\Support\OnlyTalentsContext::isActive()) {
    require $root . '/config/routes-onlytalents.php';
} else {
    require $root . '/config/routes-20260621-memberships-v1.php';
}
$ref = new ReflectionClass($router);
$prop = $ref->getProperty('routes');
$prop->setAccessible(true);
$routes = $prop->getValue($router);
$match = 'no';
foreach ($routes as $r) {
    if (($r['method'] ?? '') === 'GET' && preg_match($r['regex'], '/only-talents')) {
        $match = ($r['action'][0] ?? '') . '::' . ($r['action'][1] ?? '');
        break;
    }
}
echo "route_match=/only-talents => {$match}\n";
echo 'marker=ot-immersive-v5-purge-' . date('c') . "\n";
