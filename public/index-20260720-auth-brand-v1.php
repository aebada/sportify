<?php
if (!headers_sent()) { header('X-Sportify-FC: auth-brand-v1'); }
/**
...
$__rxHost = strtolower(preg_replace('/:\d+$/', '', (string) ($_SERVER['HTTP_HOST'] ?? '')) ?: '');
if (in_array($__rxHost, ['sportifyplus.de', 'www.sportifyplus.de'], true)
    && preg_match('#^/only-talents/home/?$#i', $__rxUri)
    && in_array(strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET')), ['GET', 'HEAD'], true)) {
    $__rxQs = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_QUERY);
    $__rxTarget = '/only-talents' . (is_string($__rxQs) && $__rxQs !== '' ? '?' . $__rxQs : '');
    header('Location: ' . $__rxTarget, true, 301);
    header('Cache-Control: no-store');
    header('X-Sportify-Ot-Canonical: index-20260707');
    exit;
}
...
// deploy-marker:20260720-auth-brand-ob-v1
$__authBrandPaths = ['/login', '/register', '/forgot-password'];
$__authBrandActive = in_array($path, $__authBrandPaths, true)
    || str_starts_with($path, '/reset-password')
    || str_starts_with($path, '/password/');
if (!empty($__authBrandActive)) {
    ob_start();
}
...
if (!empty($__authBrandActive)) {
    $__authHtml = (string) ob_get_clean();
    if ($__authHtml !== '' && !str_contains($__authHtml, 'class="auth-brand')) {
        $__brand = "<!-- deploy-marker:20260720-auth-brand-topleft-v1 -->\n"
            ...
            . '</a>';
        if (preg_match('/<body([^>]*)>/i', $__authHtml)) {
            $__authHtml = preg_replace('/<body([^>]*)>/i', '<body$1>' . $__brand, $__authHtml, 1);
        } else {
            $__authHtml = $__brand . $__authHtml;
        }
        if (!headers_sent()) {
            header('X-Sportify-Auth-Brand: ob-v1');
        }
    }
    echo $__authHtml;
}
