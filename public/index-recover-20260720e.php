<?php
if (!headers_sent()) { header('X-Sportify-FC: recover-20260720e'); }

/**
 * Sportify front controller — RefereeX live entry (2026-07-08).
 * deploy-marker: index-refereex-live-v20260708
 */
...
if (in_array($__rxHost, ['sportifyplus.de', 'www.sportifyplus.de'], true)
    ...
    header('X-Sportify-Ot-Canonical: index-20260708');
    exit;
}
...
if (function_exists('opcache_invalidate')) {
    foreach ([
        ...
        $root . '/app/Controllers/FitPassController.php',
...
if ($path === '/refereex-ai' || str_starts_with($path, '/refereex-ai/')
    ...
    || $path === '/refereex-ai.html' || str_ends_with($path, '/refereex-ai.html')) {
    header('Location: https://aebada.github.io/sportify-refereex/', true, 302);
    header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
    header('X-Sportify-RefereeX: index-ghpages-redirect-20260715');
    exit;
}
