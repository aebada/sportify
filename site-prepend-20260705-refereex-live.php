...
if (!headers_sent()) {
    header('X-Sportify-Ot-Prepend: 20260707');
}

$__sportifyMainHost = strtolower(preg_replace('/:\d+$/', '', (string) ($_SERVER['HTTP_HOST'] ?? '')) ?: '');
$__otCanonicalPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?: '/';
if (in_array($__sportifyMainHost, ['sportifyplus.de', 'www.sportifyplus.de'], true)
    && preg_match('#^/only-talents/home/?$#i', $__otCanonicalPath)
    && in_array(strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET')), ['GET', 'HEAD'], true)) {
    $__otQs = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_QUERY);
    $__otTarget = '/only-talents' . (is_string($__otQs) && $__otQs !== '' ? '?' . $__otQs : '');
    header('Location: ' . $__otTarget, true, 301);
...
$__rxRefereex = ($__p === '/refereex-ai' || str_starts_with($__p, '/refereex-ai/') || !empty($_GET['refereex']));
if ($__rxRefereex) {
    $__root = is_file(__DIR__ . '/bootstrap-20260621-authfix.php')
        ? __DIR__
        : (is_file(dirname(__DIR__) . '/bootstrap-20260621-authfix.php') ? dirname(__DIR__) : __DIR__);
    http_response_code(200);
    header('Content-Type: text/html; charset=UTF-8');
    header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
    header('X-Sportify-RefereeX: prepend-20260706-refereex-v2');
    require $__root . '/bootstrap-20260621-authfix.php';
...
if (in_array($__sportifyOtHost, ['talents.sportifyplus.de', 'onlytalents.sportifyplus.de'], true)) {
    ...
    if (in_array($otUri, ['/', '/index.php', '/public/index.php'], true) && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET') {
        header('Location: https://sportifyplus.de/only-talents', true, 302);
        header('Cache-Control: no-store');
        exit;
    }
...
ob_start(static function (string $buffer): string {
    ...
    // Sportify logo top-left on auth pages (login/register) — mobile hides .auth-side
